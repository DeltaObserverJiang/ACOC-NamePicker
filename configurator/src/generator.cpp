...
std::string RosterConst(const std::vector<roster::Student>& v,
                        const std::string& cn) {
    std::string s = "        /* 本班名单：由配置器写入 */\n";
    s += "        const CLASS_NAME = " + JsString(cn) + ";\n";
    s += "        const CLASS_NAMES = [\n";
...
bool Build(const std::string& tpl, const Config& c, std::string& out,
    ...
    const std::string cn = c.className;
    std::string btn = Pad(12) + "<button class=\"class-btn highlight\" id=\"loadCustomBtn\">"
                      "装载预设 " + cn + "</button>";
    std::string listener =
        Pad(8) + "document.getElementById('loadCustomBtn').addEventListener('click', "
    ...
    ReplaceAll(out, "@@ROSTER_CONST@@", RosterConst(c.students, cn));
